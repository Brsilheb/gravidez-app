...
def render_sentir():
    ...
    emocao = st.selectbox("Como você está se sentindo hoje?", ["Alegria","Gratidão","Ansiedade","Medo","Esperança","Ternura"])
    ...
    if st.button("Salvar registro"):
        if texto.strip():
            df = load_diario()
            new_row = pd.DataFrame([{"data": str(date.today()), "emocao": emocao, "titulo": titulo.strip(), "texto": texto.strip(), "tipo": tipo}])
